...
(function (factory) {
    if (typeof define === 'function' && define.amd) {
        ...
        define(['jquery','urlUtil'], factory);
    } else if (typeof exports === 'object') {
        // Node / CommonJS
        factory(require('jquery'),require('urlUtil'));
    } else {
        // Browser globals.
        factory(jQuery,urlUtil);
    }
})(function ($,urlUtil) {
    ...
    CityPicker.prototype = {
        ...
        refresh: function (force) {
            ...
            $.each(this.dems, $.proxy(function (i, type) {
                ...
                type===PROVINCE&&this.output(type);
            }, this));
            this.tab(PROVINCE);
            this.feedText();
            this.feedVal();
        ...
        close: function (blur) {
            this.$dropdown.is(':visible') && $.isFunction(this.options.beforeClose) && this.options.beforeClose(this);
        ...
        output: function (type) {
            ...
            if(type===PROVINCE){
                params={queryType:1,bankCode:bankCode};
                field='province';
            }else if(type===CITY){
                self.provinceCode=code;
                params={queryType:2,bankCode:bankCode,provinceCode:self.provinceCode}
                field='city';
            }else if(type===DISTRICT){
                self.cityCode=code;
                params={queryType:3,bankCode:bankCode,provinceCode:self.provinceCode,cityCode:self.cityCode}
                field='city';
            }
            params&&this.getData(params,function (result) {
                $select.html(self.getList(result, type,field));
            });
            ...
            $.each(data, function (i, n) {
                ...
                $.each(n, function (j, m) {
                    list.push(
                        ...
                        (m.selected ? ' active' : '') +
                        '">' +
                        ( simple ? $this.simplize(m.name, PROVINCE) : m.name) +
                        '</a>');
                });
                list.push('</dd></dl>');
            });
        ...
        getList: function (data, type,field) {
            ...
            $.each(data, function (i, n) {
                var name=n[field+'Name'];
                var code=n[field+'Code'];
                list.push(
                    '<a' +
                    ' title="' + (name || '') + '"' +
                    ' data-code="' + (code || '') + '"' +
                    ...
                    (n.selected ? ' active' : '') +
                    '">' +
                    ( simple ? $this.simplize(name, type) : name) +
                    '</a>');
            });
            list.push('</dd></dl>');
